...
void hud::HP_info_drawer::operator()()
{
    ...
    shared_ptr<message_writer_for_thread> temp_writer{
        new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{100, 20}}};
    funcs_.push_back([&, temp_writer] { draw_text_stage_number_1(temp_writer); });

    temp_writer.reset( new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{30, 20}});
    funcs_.push_back([&, temp_writer] { draw_text_stage_number_2(temp_writer); });

    temp_writer.reset(new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{40, 20}});
    funcs_.push_back([&, temp_writer] { draw_text_time_1(temp_writer); });

    temp_writer.reset( new_crt message_writer_for_thread{constants::FONT_NAME, 30, cv::Size{30, 30}});
    funcs_.push_back([&, temp_writer] { draw_text_time_2(temp_writer); }); 

    temp_writer.reset( new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{90, 20}});
    ...
    temp_writer.reset( new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{50, 20}});
    funcs_.push_back([&, temp_writer] { draw_text_HP_2(temp_writer); });

    temp_writer.reset( new_crt message_writer_for_thread{constants::FONT_NAME, 20, cv::Size{70, 20}});
