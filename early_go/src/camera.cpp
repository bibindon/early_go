...
camera::camera(const D3DXVECTOR3 &eye_position, const D3DXVECTOR3 &look_at_position)
...
bool camera::flexible_animation::operator()(camera &a_camera)
{
    ...
    float sine_curve { get_sine_curve(animation_count_ * constants::ANIMATION_SPEED, duration_) };

    a_camera.eye_position_ = start_eye_position_ + delta_eye_position_ * sine_curve;

    a_camera.look_at_position_ = start_look_at_position_ + delta_look_at_position_ * sine_curve;
...
void camera::set_to_close_up_animation(const D3DXVECTOR3 &pos)
{
    ...
    std::shared_ptr<key_animation> tmp = std::shared_ptr<key_animation>(
        new_crt orbit
        {
            D3DXVECTOR3 { pos.x + 0.0f, 2.0f, pos.z - 3.95f },
...
bool camera::transfer::operator()(camera &camera)
{
    ...
    camera.look_at_position_ = start_look_at_position_ + delta_look_at_position_ * sine_curve;
...
bool camera::orbit::operator()(camera &camera)
{
    ...
    float current_rotate_angle { start_rotate_angle_ + delta_rotate_angle_ * sine_curve };

    camera.eye_position_.x = std::cos(current_rotate_angle) * radius_ + center_coordinate_.x;

    camera.eye_position_.y = start_eye_position_.y;

    camera.eye_position_.z = std::sin(current_rotate_angle) * radius_ + center_coordinate_.z;
