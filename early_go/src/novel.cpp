...
std::deque<
    std::tuple<std::string, std::string, bool> > novel::portrait_order_;
...
int novel::glue_draw_portrait(lua_State* L) {
  ...
  if (std::strcmp(position, "center") != 0 &&
      std::strcmp(position, "left") != 0 &&
      std::strcmp(position, "right") != 0) {
      return 0;
  }
  auto it = std::remove_if(portrait_order_.begin(), portrait_order_.end(),
      [&](auto& x){return std::get<1>(x) == position;});
  if (it != portrait_order_.end()) {
    portrait_order_.erase(it);
  }

  portrait_order_.push_front(std::make_tuple(filename, position, false));
...
int novel::glue_draw_portrait_flip(lua_State* L) {
  ...
  portrait_order_.push_front(std::make_tuple(filename, position, true));
...
void novel::redraw_portrait()
{
  ...
  for (int layer = 4; layer >= 2; --layer, ++it) {
    if (it == portrait_order_.cend()) {
      window_->get_main_character()->clear_dynamic_texture(
          constants::EARLY_BODY, layer);
      break;
    }
    window_->get_main_character()->set_dynamic_texture(
    ...
    } else if (std::get<1>(*it) == "left") {
      window_->get_main_character()->set_dynamic_texture_position(
          constants::EARLY_BODY, layer, ::D3DXVECTOR2{0.1f, 0.0f});
    } else if (std::get<1>(*it) == "right") {
      window_->get_main_character()->set_dynamic_texture_position(
          constants::EARLY_BODY, layer, ::D3DXVECTOR2{-0.3f, 0.0f});
    }
    if (std::get<2>(*it)) {
      window_->get_main_character()->flip_dynamic_texture(
          constants::EARLY_BODY, layer);
    }
  }
}
...
void novel::operator()(basic_window& window)
{
  ...
  if (key::is_down('K')) {
    ...
    for (int i = 0; i < 8; ++i) {
      if (!window_->get_main_character()->
            is_tex_animation_finished(constants::EARLY_BODY, i)) {
